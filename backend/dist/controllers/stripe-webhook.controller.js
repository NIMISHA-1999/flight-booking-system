...
const stripeWebhook = async (req, res) => {
    ...
    if (event.type === "checkout.session.completed") {
        try {
            ...
            if (!bookingId) {
                console.error("❌ bookingId NOT FOUND IN CHECKOUT SESSION");
    ...
    if (event.type === "payment_intent.payment_failed") {
        try {
            ...
            if (bookingId) {
                await database_1.prisma.booking.updateMany({
                    where: {
                        id: bookingId,
                        status: "PENDING",
                    },
                ...
                console.log("❌ BOOKING MARKED PAYMENT_FAILED");
            }
        }
        catch (error) {
    ...
    if (event.type === "checkout.session.expired") {
        try {
            const session = event.data.object;
            const bookingId = session.metadata?.bookingId;
            console.log("CHECKOUT EXPIRED:", bookingId);
            if (bookingId) {
                await database_1.prisma.booking.updateMany({
                    where: {
                        id: bookingId,
            ...
            console.error("❌ CHECKOUT EXPIRED ERROR:", error);
        }
    }
    // =====================================================
